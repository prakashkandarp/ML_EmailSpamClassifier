...
if len(feature_names) != 58:
    st.error(
        f"Error: Feature name mismatch: Expected 58 names, found {len(feature_names)}. "
    )
    st.stop()
...
st.sidebar.markdown("---")
run_clicked = st.sidebar.button("Run Evaluation", use_container_width=True)
...
st.sidebar.download_button(
    label="Download Sample Test CSV (50 rows)",
    data=csv_bytes,
...
if uploaded_file is not None:
    try:
        ...
        if data.shape[1] != 58:
            st.error(
                f"Error: Invalid file format: Expected 58 columns (57 features + 1 label), "
                f"but found {data.shape[1]} columns."
            )
            st.stop()

    except Exception:
        st.error("Error: Unable to read the uploaded file. Please upload a valid CSV.")
        st.stop()
...
if show_data:
    if data is None:
        st.warning("Warning: Please upload a CSV file first.")
    else:
...
if run_clicked:
    if data is None:
        st.warning("Warning: Please upload a test CSV file before running evaluation.")

    elif model_name == "Please select a model":
        st.warning("Warning: Please select a machine learning model before running evaluation.")
        ...
        if model_name in ["random_forest", "xgboost"]:
            ...
            with st.expander("Feature Contribution Explanation"):
                st.markdown(
    ...
    st.info(
        "Please Select a model, upload a test CSV file, and click **Run Evaluation** "
